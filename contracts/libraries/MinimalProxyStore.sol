...
library MinimalProxyStore {
     ...
     * @return context the queried context data
     */
    function getContext(
        address instance
        ...
        uint256 start = 46;
        uint256 size = instanceCodeLength - start;
        ...
        assembly {
            ...
            mstore(
                0x40,
                add(context, and(add(add(size, 0x20), 0x1f), not(0x1f)))
            )
            // store length in memory
            mstore(context, size)
            // actually retrieve the code, this needs assembly
            extcodecopy(instance, add(context, 0x20), start, size)
        }
    }
